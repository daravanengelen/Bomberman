...
void GameOverState::render(Game& game, sf::RenderTarget& target) {
    ...
    if (game.hasFont()) {
        drawCenteredText(target, game.getFont(), won ? "You Win!" : "You Lose!", 44, titleColor, centerX, 220.f);
        std::ostringstream scoreLine;
        scoreLine << "Final Score: " << game.getScore()->getCurrentScore();
        drawCenteredText(target, game.getFont(), scoreLine.str(), 26, sf::Color::White, centerX, 300.f);
        drawCenteredText(target, game.getFont(), "Press Enter to return to menu", 20, sf::Color(200, 200, 210), centerX,
                         420.f);
    } else {
