...
std::shared_ptr<Logic::Wall> GameFactory::createWall(unsigned int id, const Logic::Vector2& position,
                                                     const Logic::Vector2& halfExtents, bool indestructible) {
    auto wall = std::make_shared<Logic::Wall>(id, position, halfExtents, indestructible);
    attachView(wall);
    return wall;
}

std::shared_ptr<Logic::Character> GameFactory::createCharacter(unsigned int id, const Logic::Vector2& position,
                                                               const Logic::Vector2& halfExtents, float moveSpeed,
                                                               bool isPlayer) {
    auto character = std::make_shared<Logic::Character>(id, position, halfExtents, moveSpeed, isPlayer);
...
std::shared_ptr<Logic::Bomb> GameFactory::createBomb(unsigned int id, const Logic::Vector2& position,
    ...
    auto bomb = std::make_shared<Logic::Bomb>(id, position, halfExtents, fuseDuration, blastRadius, ownerId);
    attachView(bomb);
    return bomb;
}

std::shared_ptr<Logic::PowerUp> GameFactory::createPowerUp(unsigned int id, const Logic::Vector2& position,
...
void GameFactory::clearViews() { m_views.clear(); }
